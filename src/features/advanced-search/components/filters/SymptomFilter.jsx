...
import { SYMPTOM_LIST } from "../../utils/searchConstants";

/**
 * Filtre par symptômes avec recherche et tags
 */
function SymptomFilter({ activeFilters, onToggle }) {
  const [search, setSearch] = useState("");

  const filteredSymptoms = useMemo(() => {
    if (!search.trim()) return SYMPTOM_LIST;
    const normalized = search.toLowerCase().trim();
    return SYMPTOM_LIST.filter((symptom) =>
      symptom.label.toLowerCase().includes(normalized),
    );
  }, [search]);

  const selectedSymptoms = useMemo(() => {
    return SYMPTOM_LIST.filter((symptom) => activeFilters[symptom.id]);
  }, [activeFilters]);
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <input
          ...
          aria-label="Filtrer la liste des symptômes"
        />
        {search && (
          <button
            onClick={() => setSearch("")}
      ...
      {/* Tags des symptômes sélectionnés */}
      {selectedSymptoms.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {selectedSymptoms.map((symptom) => (
            <button
              key={symptom.id}
              onClick={() => onToggle("symptoms", symptom.id)}
              ...
              aria-label={`Retirer ${symptom.label}`}
            >
              {symptom.label}
      ...
      <div className="max-h-40 space-y-0.5 overflow-y-auto">
        {filteredSymptoms.map((symptom) => (
          ...
          >
            <input
              type="checkbox"
              checked={activeFilters[symptom.id] || false}
              onChange={() => onToggle("symptoms", symptom.id)}
